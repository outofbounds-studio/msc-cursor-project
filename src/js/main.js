...
function ensureDependenciesLoaded() {
    return new Promise((resolve, reject) => {
        let checkCount = 0;
        const maxChecks = 50; // 5 seconds maximum wait time
...
// Wrap the entire code in an async initialization function
async function initializeWebflowCursor() {
    try {
        ...
        console.error('Failed to initialize webflow-cursor:', error);
    }
}

// Export the initialization function
window.webflowCursorProject = {
    init: function() {
        // Wrap the initialization in a try-catch for better error handling
        try {
            ensureDependenciesLoaded()
                .then(() => {
                    // Initialize only after DOM is ready
                    if (document.readyState === 'loading') {
                        document.addEventListener('DOMContentLoaded', initializeWebflowCursor);
                    } else {
                        initializeWebflowCursor();
                    }
                })
                .catch(error => {
                    console.error('Failed to initialize webflowCursorProject:', error);
                });
        } catch (error) {
